...
use constant BUILD => 3;
use constant COPYRIGHT_YEAR => "2003-2017";
...
console_output ( 'LogSpit Version '. VERSION . ' Build ' . BUILD );
botconfig::init();
IRC::init();
...
sub parse_line
{
	...
	#D:\Rencorner\FuckWar\Server\Renegade\FDS\renlog_1-5-2017.txt
	#my @dirname = split("\\", $filename);
	
	#Get the server name in the file path so we can use it as a prefix of a message. 
	if ( $filename =~ /^.:\\(.+?)\\(.+?)\\(.+)/) 
	{
		#print "$1 $2 $3 $4\n";
		$prefix = "[$2]";
	...
	if ( $filename =~ /renlog/i) {
			...
			|| $input =~ /Server Shutdown/ )
		{
			...
			#print "$filename\n";
			console_output("$prefix $input\n");
			IRC::ircmsg ( "$prefix $input" );
		}
	}
	else{
		print "$input\n";
