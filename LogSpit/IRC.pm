	...
	( inline_states =>
		{
			_start => sub
			{
				...
				main::console_output ( "[IRC] Connecting to IRC $botconfig::config_ircserver:$botconfig::config_ircport" );
				...
				if ( $socket && $socket->connected() )
				{
					...
					sendToServer ( "USER $botconfig::config_botname * * :ServerWatch ".main::VERSION." build ".main::BUILD );
			...
			gotline => sub
			{
				...
				if ( $input =~ m/^\:(\S+)\s001\s/ )
				{
					...
					ircmsg ( "LogSpit reporting for duty!" );
				}
				...
				elsif ( $input =~ m/^\:(\S+)\s433\s/ )
				{
					...
					sendToServer ( "USER ".$heap->{'ircNick'}." * * :LogSpit Version 1.0" );
				}
			},
			goterror => sub
			{
