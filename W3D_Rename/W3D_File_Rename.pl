...
foreach my $file (@files) 
{
  ...
  if ( $config_copy_file && $config_single_file )
  {
  ...
  if ( $config_add_name && $config_change )
  {
	  #Remove file type
...
sub ReadConfig
{
	...
	while ( <$fh> )
	{
		if ( $_ !~ m/^\#/ && $_ !~ m/^\s+$/ )		# If the line starts with a hash or is blank ignore it
		{
			...
			if (/^\s*SingleFileCopy\s*\=\s*(\S+)/i)						{ $config_single_file = $1; }
