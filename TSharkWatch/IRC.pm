...
# Variables
my $socket;
our @Queue;			# Queue for outgoing messages
our @WireShark;
	...
	( inline_states =>
		{
			_start => sub
			{
				...
				if ( $socket && $socket->connected() )
				{
					...
					main::console_output ( "[IRC] User $botconfig::config_botname :Server ".main::BR_VERSION." build ".main::BR_BUILD );
					# Identify ourself to the server
					sendToServer ( "NICK $botconfig::config_botname" );
					sendToServer ( "USER $botconfig::config_botname * * :Server ".main::BR_VERSION." build ".main::BR_BUILD );
			...
			sendloop => sub
			{
				...
				foreach my $loop_variable (@WireShark) { 
					print $loop_variable;
				}
				...
				if ( $WireShark[0] )
				{
					my $output = shift @WireShark;
					$sendString .= $output;
					$availableChars -= length ( $output );
				}

				# Keep sending messages until we hit out chars per second limit
				while ( $WireShark[0] && ( length($WireShark[0]) < $availableChars ) )
				{
					my $output = shift @WireShark;
					$sendString .= $output;
					$availableChars -= length ( $output );
...
sub sendToServer
{
	...
	#foreach my $loop_variable (@Queue) { 
	#	print $loop_variable;
	#} 
}
...
sub ircmsg
{
	...
	#{
	#	print "$line";
	#	sendToServer ( "PRIVMSG $botconfig::irc_adminChannel :$line");
	#}
