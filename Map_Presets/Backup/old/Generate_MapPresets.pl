...
print "$dirname.cfg\n";
our $configfile					= "./generate_mappresets.cfg";
our $config_preset_file 		= "./temp_objects";
...
if ( -e "$config_sounds_file" )
{
	unlink "$config_sounds_file";
}

if ( -e "./jukebox_xml.txt" )
{
	unlink "./jukebox_xml.txt";
}
...
foreach(@sounds)
{
	...
	if ( $config_jukebox )
	{
		open ( SOUNDSFILE, '>>.\jukebox_xml.txt' );
