...
sub WriteCommandsRef($$)
{
	...
	{
		my $msg = "WriteCommandsRef Error Array Parameter Not an array Reference\n";
		print($msg);
		chomp $msg;
		WriteFile($file, $msg);
		return;
...
sub PrintCommandsRef($)
{
	...
	if ( ref($array) ne 'ARRAY' )
	{
		my $msg = "PrintCommandsHash Error Array Parameter Not an array Reference\n";
		print($msg);
		chomp $msg;
...
#Sort by Array Single String method
sub ArrangeByType
{
	my $array = shift;
	my $type = shift;
